...
function transactionTypeLabel(type) {
  ...
  if (t.includes('create') || t.includes('trial') || t.includes('renew') || t.includes('ssh') || t.includes('vmess') || t.includes('vless') || t.includes('trojan') || t.includes('shadowsocks')) {
...
bot.action('cek_service', async (ctx) => {
  try {
    ...
    if (!isReseller && !isAdmin) {
      return showPublicServerStatus(ctx);
    }
...
processQrisTopupInvoice = async function processQrisTopupInvoice(ctx, baseAmount, forcedUniqueSuffix = null) {
  ...
  try {
    ...
    if (IS_PRIMARY_INSTANCE && !global.__qrisPollStarted) {
      ...
      logger.info(`ℹ️ QRIS polling aktif. Interval=${Number(QRIS_CHECK_INTERVAL_MS || 15000)}ms`);
    } else if (!IS_PRIMARY_INSTANCE) {
      logger.info('⚠️ QRIS polling nonaktif di instance non-primary (PM2 cluster).');
    }
