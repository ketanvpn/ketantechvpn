...
function buildEditFieldCancelText(fieldName = '') {
  return '⛔ *Edit ' + fieldName + ' dibatalkan.*';
}

function buildEditFieldSuccessText(options = {}) {
  const {
    fieldName = '',
    serverName = '',
    oldValue = '-',
...
function buildEditFieldInputText(options = {}) {
  const {
    fieldName = '',
    serverName = '',
    oldValue = '-',
  ...
  return (
    '✏️ *Edit ' + fieldName + '*\n\n' +
    '📍 Server: *' + serverName + '*\n' +
    '🔢 Nilai sekarang: *' + oldValue + '*\n' +
    '🆕 Input baru: *' + currentValue + '*\n\n' +
    '_Tekan ✅ untuk simpan atau ❌ Batal untuk membatalkan._'
  );
}
